...
from transformers import Adafactor, T5ForConditionalGeneration, T5Tokenizer, LongT5ForConditionalGeneration, \
    AutoTokenizer, AdamW
...
def init_args():
    ...
    parser.add_argument("--dataset", default='fbst', type=str, required=True,
                        help="The name of the dataset, selected from: [laptop14, rest14, rest15, rest16]")

    parser.add_argument("--model_name_or_path", default='google/flan-t5-base', type=str,
    ...
    # Other parameters
    parser.add_argument("--max_seq_length", default=756, type=int)
...
class T5FineTuner(pl.LightningModule):
    ...
    def _step(self, batch):
        # lm_labels = batch["src_target_ids"]
        # lm_labels[lm_labels[:, :] == self.tokenizer.pad_token_id] = -100
        #
        # outputs = self(
        #     input_ids=batch["source_st_ids"],
        #     attention_mask=batch["src_st_mask"],
        #     labels=lm_labels,
        #     decoder_attention_mask=batch['src_target_mask']
        # )
        # st_loss = outputs[0]

        lm_labels = batch["triplet_ids"]
        lm_labels[lm_labels[:, :] == self.tokenizer.pad_token_id] = -100
        ...
        outputs = self(
            ...
            labels=lm_labels,
            decoder_attention_mask=batch["triplet_mask"]
        )
        loss = outputs[0]

        # lm_labels = batch["source_only_target_ids"]
        # lm_labels[lm_labels[:, :] == self.tokenizer.pad_token_id] = -100
        #
        # outputs = self(
        #     input_ids=batch["source_only_text_ids"],
        #     attention_mask=batch["source_only_text_mask"],
        #     labels=lm_labels,
        #     decoder_attention_mask=batch["source_only_target_mask"]
        # )
        # s_loss = outputs[0]

        return loss  # + st_loss + s_loss
    ...
    def validation_epoch_end(self, outputs):
        ...
        sents, _ = read_line_examples_from_file(f'data/{args.task}/{args.dataset}/test.csv')
        raw_scores, fixed_scores = evaluate(test_loader, model, args.paradigm, args.task, sents)
        f1 = torch.tensor(raw_scores['f1'])
        return {"avg_val_loss": avg_loss, "log": tensorboard_logs, 'progress_bar': tensorboard_logs, 'f1': f1}
    ...
    def get_tqdm_dict(self):
        tqdm_dict = {"loss": "{:.4f}".format(self.trainer.avg_loss)}
        return tqdm_dict
    ...
    def val_dataloader(self):
        ...
        return DataLoader(val_dataset, batch_size=self.hparams.eval_batch_size, num_workers=2, shuffle=True)
...
def evaluate(data_loader, model, paradigm, task, sents):
    ...
    for batch in tqdm(data_loader):
        ...
        target = [tokenizer.decode(ids, skip_special_tokens=True) for ids in batch["triplet_ids"]]

        outputs.extend(dec)
        targets.extend(target)

    raw_scores, fixed_scores = compute_scores(outputs, targets, sents, paradigm, seed=args.seed, task=args.dataset)
    # results = {'raw_scores': raw_scores
    # , 'fixed_scores': fixed_scores, 'labels': all_labels,
...
print('Output:', tokenizer.decode(data_sample['triplet_ids'], skip_special_tokens=True))
...
if args.do_train:
    ...
    checkpoint_callback = pl.callbacks.ModelCheckpoint(
        filepath=args.output_dir, prefix="ckt", monitor='f1', mode='max', save_top_k=3
    )
    ...
    train_params = dict(
        ...
        callbacks=[pl.callbacks.EarlyStopping(monitor='f1', patience=3, mode='max')],
    )
...
if args.do_direct_eval:
    ...
    # write to file
    log_file_path = f"/home/jmurzaku/flant5/results_log/{args.task}-{args.dataset}.txt"
    ...
    exp_results = f"Raw F1 = {raw_scores:.4f}, Fixed F1 = {fixed_scores:.4f}"
    log_str = f'============================================================\n'
