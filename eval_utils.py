    ...
    for i, char in enumerate(string):
        if char == '(':
            ...
            if stack:
                start = stack.pop()
            if not stack:
                values.append(string[start:i + 1])
...
def compute_f1_scores(pred_pt, gold_pt):
    ...
    for g_doc in gold_pt:
        g_doc = (g_doc.replace('(', ' (').replace('<src>', 'src'))
    ...
    if '(' in pred_pt[0]:
        ...
        return {'f1': 0}
def compute_scores(pred_seqs, gold_seqs, sents, io_format, seed, task):
    """
    compute metrics for multiple tasks
    """
    print("\nResults of raw output")
    raw_scores = compute_f1_scores(pred_seqs, gold_seqs)
    import pandas as pd
    df = pd.DataFrame()
    df['sentence'] = [' '.join(i) for i in sents]
    df['gold'] = gold_seqs
    df['pred'] = pred_seqs
    df.to_csv(f"preds_raw{io_format}{seed}{task}.csv")
    print(raw_scores)
